...
Visualizer::EvaluationLocator::EvaluationLocator(Vrui::LocatorTool* sLocatorTool,Visualizer* sApplication,const char* dialogWindowTitle)
	...
	posBox->setOrientation(GLMotif::RowColumn::HORIZONTAL);
	posBox->setPacking(GLMotif::RowColumn::PACK_GRID);
	
	for(int i=0;i<3;++i)
		{
		char labelName[40];
		snprintf(labelName,sizeof(labelName),"Pos-%d",i+1);
		pos[i]=new GLMotif::TextField(labelName,posBox,12);
