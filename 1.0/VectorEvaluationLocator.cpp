...
Visualizer::VectorEvaluationLocator::VectorEvaluationLocator(Vrui::LocatorTool* sLocatorTool,Visualizer* sApplication)
	:EvaluationLocator(sLocatorTool,sApplication,"Vector Evaluation Dialog"),
	 ...
	 colorMap(application->colorMaps[application->scalarVariable]),
	 vectorExtractor(application->vectorExtractor->clone()),
	 valueValid(false),
	 arrowLengthScale(1)
	{
	/* Get the style sheet: */
	...
	new GLMotif::Label("ValueLabel",evaluationDialog,application->dataSet->getVectorVariableName(application->vectorVariable));
