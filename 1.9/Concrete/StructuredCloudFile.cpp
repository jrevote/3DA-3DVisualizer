/***********************************************************************
StructuredCloudFile - Class defining lowest-common-denominator ASCII
...
#include <Concrete/StructuredCloudFile.h>
...
/************************************
Methods of class StructuredCloudFile:
************************************/

StructuredCloudFile::StructuredCloudFile(void)
	:BaseModule("StructuredCloudFile")
	{
	}

Visualization::Abstract::DataSet* StructuredCloudFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	{
	...
	while(parsedHeaderLines < 2)
		...
		if(gridFile.eof())
			Misc::throwStdErr("StructuredCloudFile::load: early end-of-file in grid file %s",argIt->c_str());
				...
				case 0:
					if(strncasecmp(line, "gridsize" ,8) == 0 && (line[8] == ' ' || line[8] == '\t'))
						...
						if(sscanf(line+9, "%d %d %d", &numVertices[0], &numVertices[1], &numVertices[2]) !=3)
							Misc::throwStdErr("StructuredCloudFile::load: invalid grid size in line %u in grid file %s", lineIndex, argIt->c_str());
						}
					else
						Misc::throwStdErr("StructuredCloudFile::load: missing grid size in line %u in grid file %s", lineIndex, argIt->c_str());
					break;
				...
				case 1:
					if(strncasecmp(line,"coordinate", 10) == 0 && (line[10] == ' ' || line[10] == '\t'))
						...
						if(modeEndPtr-modeStartPtr == 9 && strncasecmp(modeStartPtr, "Cartesian" ,9) == 0)
							...
							Misc::throwStdErr("StructuredCloudFile::load: invalid coordinate mode in line %u in grid file %s", lineIndex, argIt->c_str());
						}
					else
						Misc::throwStdErr("StructuredCloudFile::load: missing coordinate mode in line %u in grid file %s", lineIndex, argIt->c_str());
					break;
				}
	...
	if(sphericalCoordinates&&storeSphericals)
		...
		static const char* coordSliceNames[3] = {"Longitude", "Latitude", "Height"};
		for(int i=0; i<3; ++i)
			{
			dataSet.addSlice();
	...
	const double a=6378.14e3; // Equatorial height in m
	...
	while(index[2] < numVertices[2])
		...
		if(line[0] != '#')
			...
			if(sphericalCoordinates)
				...
				if(sscanf(line,"%lf %lf %lf", &longitude, &latitude, &height)!=3)
					Misc::throwStdErr("StructuredCloudFile::load: Invalid spherical vertex coordinate in line %u in grid file %s",lineIndex,argIt->c_str());
			
            std::cout<<"Parsing Longitude: "<<longitude<<" Latitude: "<<latitude<<" Height: "<<height<<"\n"<<std::flush;	
            ...
            double r = (a * (1.0 - f * Math::sqr(s0)) * height) * scaleFactor;
				double xy = r * c0;
            ...
            std::cout<<"Converted Longitude: "<<vertex[0]<< " Latitude: "<<vertex[1]<<" Height: "<<vertex[2]<<"\n"<<std::flush;
				
				if(storeSphericals)
					{
					/* Store the spherical coordinate components in the first three value slices: */
					dataSet.getVertexValue(0, index) = Scalar(Math::deg(longitude));
					dataSet.getVertexValue(1, index) = Scalar(Math::deg(latitude));
				...
				if(sscanf(line, "%f %f %f", &vertex[0], &vertex[1], &vertex[2]) != 3)
					Misc::throwStdErr("StructuredCloudFile::load: Invalid Cartesian vertex coordinate in line %u in grid file %s", lineIndex, argIt->c_str());
				}
   ...
   //std::cout<<"\nFinished reading "<<lineIndex<<" vertices ...\n"<<std::flush;
	...
	for(++argIt; argIt != args.end(); ++argIt)
		...
		if(strcasecmp(argIt->c_str(), "-log") == 0)
			...
			while(parsedHeaderLines < 2)
				...
				/* Check if the file is already over: */
				if(sliceFile.eof())
					Misc::throwStdErr("StructuredCloudFile::load: early end-of-file in slice file %s", argIt->c_str());
						...
						case 0:
							if(strncasecmp(line, "gridsize", 8) == 0 && (line[8] == ' ' || line[8] == '\t'))
								...
								if(sscanf(line+9, "%d %d %d", &sliceNumVertices[0], &sliceNumVertices[1], &sliceNumVertices[2]) != 3)
									Misc::throwStdErr("StructuredCloudFile::load: invalid grid size in line %u in slice file %s", lineIndex,argIt->c_str());
								
								/* Check if the slice grid size matches the grid size: */
								if(sliceNumVertices != numVertices)
									Misc::throwStdErr("StructuredCloudFile::load: mismatching grid size in slice file %s", argIt->c_str());
								}
							else
								Misc::throwStdErr("StructuredCloudFile::load: missing grid size in line %u in slice file %s", lineIndex,argIt->c_str());
							break;
							...
							if(strncasecmp(line, "scalar", 6) == 0 && (line[6] == ' ' || line[6] == '\t'))
								...
								if(nameStartPtr != nameEndPtr)
									...
									*nameEndPtr = '\0';
									...
									Misc::throwStdErr("StructuredCloudFile::load: empty vector variable name in line %u in slice file %s", lineIndex, argIt->c_str());
								}
								...
								Misc::throwStdErr("StructuredCloudFile::load: missing value type in line %u in slice file %s", lineIndex, argIt->c_str());
							break;
						}
				...
				if(line[0] != '#')
					...
					if(vectorValue)
						...
						if(sphericalCoordinates)
							...
							double longitude, latitude, height;
							if(sscanf(line, "%lf %lf %lf", &longitude, &latitude, &height) != 3)
								Misc::throwStdErr("StructuredCloudFile::load: Invalid spherical vector attribute in line %u in slice file %s", lineIndex, argIt->c_str());
							...
							vector[0] = Scalar(c1 * (c0 * height - s0 * latitude) - s1 * longitude);
							vector[1] = Scalar(s1 * (c0 * height - s0 * latitude) + c1 * longitude);
							vector[2] = Scalar(c0 * latitude + s0 * height);
							}
						else
							{
							/* Read the vector attribute in Cartesian coordinates: */
							if(sscanf(line, "%f %f %f", &vector[0], &vector[1], &vector[2]) != 3)
								Misc::throwStdErr("StructuredCloudFile::load: Invalid Cartesian vector attribute in line %u in slice file %s", lineIndex, argIt->c_str());
							}
						...
						if(logNextScalar)
							...
							if(sscanf(line, "%lf", &value) != 1)
								Misc::throwStdErr("StructuredCloudFile::load: Invalid logarithmic scalar vertex attribute in line %u in slice file %s",lineIndex,argIt->c_str());
							...
							if(sscanf(line, "%f", &dataSet.getVertexValue(sliceIndex, index)) != 1)
								Misc::throwStdErr("StructuredCloudFile::load: Invalid scalar vertex attribute in line %u in slice file %s", lineIndex, argIt->c_str());
							}
						}
...
extern "C" Visualization::Abstract::Module* createFactory(Plugins::FactoryManager<Visualization::Abstract::Module>& manager)
	...
	Visualization::Concrete::StructuredCloudFile* module = new Visualization::Concrete::StructuredCloudFile();
