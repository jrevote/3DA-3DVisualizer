...
TriangleSet<VertexParam>::addNewChunk(
	...
	if(pipe!=0&&tail!=0)
		...
		pipe->write<Vertex>(tail->vertices,chunkSize*3);
		pipe->finishMessage();
		}
...
inline
void
TriangleSet<VertexParam>::finish(
	void)
	{
	if(pipe!=0)
		{
		/* Send the last batch of triangles across the pipe: */
		if(tail!=0&&tailRoomLeft<chunkSize)
			{
			size_t numTailTriangles=chunkSize-tailRoomLeft;
			pipe->write<unsigned int>((unsigned int)numTailTriangles);
			pipe->write<Vertex>(tail->vertices,numTailTriangles*3);
			}
