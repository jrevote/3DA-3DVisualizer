/***********************************************************************
SphericalCloudFile - Class to read multivariate scalar data in spherical
...
#include <Concrete/SphericalCloudFile.h>
...
/***********************************
Methods of class SphericalCloudFile:
***********************************/

SphericalCloudFile::SphericalCloudFile(void)
	:BaseModule("SphericalCloudFile")
	{
	}

Visualization::Abstract::DataSet* SphericalCloudFile::load(const std::vector<std::string>& args,Comm::MulticastPipe* pipe) const
	{
		...
		if((*argIt)[0]=='-')
			...
			if(strcasecmp(argIt->c_str()+1,"headers")==0)
				...
				for(int i=0;i<3;++i)
					...
					if(speed<0||speed>=3)
						Misc::throwStdErr("SphericalCloudFile::load: Invalid node counting speed %d specified, must be 0, 1, or 2",speed);
	...
	if(dataFileName==0)
		Misc::throwStdErr("SphericalCloudFile::load: No data file name provided");
	if(numVertices.calcIncrement(-1)==0)
		Misc::throwStdErr("SphericalCloudFile::load: No number of nodes provided");
	if(numDataSlices==0)
		Misc::throwStdErr("SphericalCloudFile::load: No scalar or vector data values specified");
	...
	if(storeSphericals)
		...
		dataValue.setScalarVariableName(sphericalBaseIndex+2,"Height");
		}
	...
	for(std::vector<VectorVariable>::const_iterator vIt=vectors.begin();vIt!=vectors.end();++vIt,++vectorVariableIndex)
		{
		static const char* vectorComponentNames[7]={"Longitude","Latitude","Height","X","Y","Z"};
		for(int i=0;i<7;++i)
			{
			if(i==1)
	...
	std::cout<<"Reading grid vertex positions and values...   0%\n"<<std::flush;
	...
	for(index[nodeCountOrder[0]]=index0Min;index[nodeCountOrder[0]]!=index0Max;index[nodeCountOrder[0]]+=index0Increment)
		...
		for(index[nodeCountOrder[1]]=0;index[nodeCountOrder[1]]<numVertices[nodeCountOrder[1]];++index[nodeCountOrder[1]])
			for(index[nodeCountOrder[2]]=0;index[nodeCountOrder[2]]<numVertices[nodeCountOrder[2]];++index[nodeCountOrder[2]])
               ...
               std::cout<<"Extracted value "<<columns[i]<<" at column "<<i<<"...\n"<<std::flush;
               }
				reader.skipLine();
				reader.skipWs();
...
Visualization::Abstract::DataSetRenderer* SphericalCloudFile::getRenderer(const Visualization::Abstract::DataSet* dataSet) const
	{
...
extern "C" Visualization::Abstract::Module* createFactory(Plugins::FactoryManager<Visualization::Abstract::Module>& manager)
	...
	Visualization::Concrete::SphericalCloudFile* module=new Visualization::Concrete::SphericalCloudFile();
