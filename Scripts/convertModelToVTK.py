...
try:
   ...
   header = re.compile(r'^#Iteration\s+No.\s*(?P<iteration>\d+)\s*'
                       'RMS\s+=\s+(?P<rms>\d+\.*\d*)\s*'
                       'LM\s+=\s+(?P<lm>\d+\.*\d*)\s*$')
   ...
   try:
      ...
      for line in model_file.readlines():
         ...
         if header_match:
            ...
            print "Iteration:", int(header_match.group('iteration'))
            print "RMS:", float(header_match.group('rms'))
            print "LM:", float(header_match.group('lm'))
         ...
         # Let's read the block sizes values and cell data.   
            ...
            if read_blocks:
               ...
               if read_blocks:   
                  for size in sizes:   
                     block_sizes[coord].append(float(size)/1000.0)    
