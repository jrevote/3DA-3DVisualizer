...
class IndexedTriangleSet:public GLObject
	...
	size_t sentTailVertices; // Number of vertices in the tail chunk that were already sent across the pipe
	...
	bool receive(void); // Receives triangle set data via multicast pipe; returns true when triangle set is fully received
	...
	void finish(void); // Finishes adding triangles to the triangle set
