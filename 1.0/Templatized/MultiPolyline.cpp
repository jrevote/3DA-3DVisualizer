...
MultiPolyline<VertexParam>::addNewChunk(
	...
	if(pipe!=0&&p.tail!=0)
		...
		pipe->write<unsigned int>((unsigned int)chunkSize);
		pipe->write<Vertex>(p.tail->vertices,chunkSize);
		pipe->finishMessage();
		}
...
MultiPolyline<VertexParam>::receive(
	void)
	{
	/* Read the polyline index of the next batch: */
	unsigned int polylineIndex=pipe->read<unsigned int>();
	
	/* Stop reading if the multipolyline is fully received: */
	if(polylineIndex>=numPolylines)
		return true;
...
inline
void
MultiPolyline<VertexParam>::finish(
	void)
	{
	if(pipe!=0)
		{
		for(unsigned int polylineIndex=0;polylineIndex<numPolylines;++polylineIndex)
			...
			/* Send the last batch of vertices across the pipe: */
			if(p.tail!=0&&p.tailRoomLeft<chunkSize)
				{
				size_t numTailVertices=chunkSize-p.tailRoomLeft;
				pipe->write<unsigned int>(polylineIndex);
				pipe->write<unsigned int>((unsigned int)numTailVertices);
				pipe->write<Vertex>(p.tail->vertices,numTailVertices);
				}
			}
		
		/* Signal that the polyline is finished: */
