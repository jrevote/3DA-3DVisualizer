...
Visualization::Abstract::DataSet* CitcomtVectorFile::load(const std::vector<std::string>& args) const
	{
	/* Open the data file: */
	...
	*********************************************************/
	...
	for(DS::Array::iterator vIt=vertices.begin();vIt!=vertices.end();++vIt)
		...
		if(sphericalCoordinates)
			...
			if(logScale)
				vIt->value.scalar=float(Math::log10(columns[3]));
			else
				vIt->value.scalar=float(columns[3]);
			
			/* Convert the vector value from spherical to Cartesian coordinates: */
			vIt->value.vector[0]=float(c1*(c0*vector[2]-s0*vector[0])-s1*vector[1]);
			vIt->value.vector[1]=float(s1*(c0*vector[2]-s0*vector[0])+c1*vector[1]);
			vIt->value.vector[2]=float(c0*vector[0]+s0*vector[2]);
			}
		else
			{
			...
			if(logScale)
				...
				vIt->value.scalar=float(columns[3]);
			for(int i=0;i<3;++i)
				vIt->value.vector[i]=float(vector[i]);
			}
