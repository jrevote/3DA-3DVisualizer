...
EarthDataSet<DataSetBaseParam>::EarthDataSet(
	...
	topographyFileName=EARTHRENDERER_IMAGEDIR;
	...
	for(std::vector<std::string>::const_iterator aIt=args.begin();aIt!=args.end();++aIt)
		{
		if(strcasecmp(aIt->c_str(),"-topo")==0)
			{
			++aIt;
			if(aIt==args.end())
				break;
			topographyFileName=*aIt;
			}
		else if(strcasecmp(aIt->c_str(),"-points")==0)
			havePoints=true;
		else if(havePoints)
...
inline
const char*
EarthDataSetRenderer<DataSetBaseParam,DataSetRendererBaseParam>::getTopographyFileName(
	const Visualization::Abstract::DataSet* sDataSet)
	{
	const EarthDataSet* eds=dynamic_cast<const EarthDataSet*>(sDataSet);
	if(eds==0)
		Misc::throwStdErr("EarthDataSetRenderer::EarthDataSetRenderer: given data set object is of incompatible class");
	return eds->getTopographyFileName().c_str();
	}
...
EarthDataSetRenderer<DataSetBaseParam,DataSetRendererBaseParam>::EarthDataSetRenderer(
	...
	:DataSetRendererBase(sDataSet),
	 er(1.0e-3,getTopographyFileName(sDataSet)),
