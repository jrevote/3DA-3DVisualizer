...
Polyline<VertexParam>::addNewChunk(
	...
	if(pipe!=0&&tail!=0)
		...
		pipe->write<Vertex>(tail->vertices,chunkSize);
		pipe->finishMessage();
		}
...
Polyline<VertexParam>::receive(
	...
	/* Stop reading if polyline is fully received: */
	if(numBatchVertices==0)
		return true;
...
inline
void
Polyline<VertexParam>::finish(
	void)
	{
	if(pipe!=0)
		{
		/* Send the last batch of vertices across the pipe: */
		if(tail!=0&&tailRoomLeft<chunkSize)
			{
			size_t numTailVertices=chunkSize-tailRoomLeft;
			pipe->write<unsigned int>((unsigned int)numTailVertices);
			pipe->write<Vertex>(tail->vertices,numTailVertices);
			}
