...
#include <Concrete/StructuredGocadFile.h>
...
/************************************
Methods of class StructuredGocadFile:
************************************/

StructuredGocadFile::StructuredGocadFile(void)
	:BaseModule("StructuredGocadFile")
	{
	}

Visualization::Abstract::DataSet* StructuredGocadFile::load(const std::vector<std::string>& args,Comm::MulticastPipe* pipe) const
	{
	...
	while(parsedHeaderLines < 2)
		...
		if(gridFile.eof())
			Misc::throwStdErr("StructuredGocadFile::load: early end-of-file in grid file %s",argIt->c_str());
				...
				case 0:
					if(strncasecmp(line, "gridsize" ,8) == 0 && (line[8] == ' ' || line[8] == '\t'))
						...
						if(sscanf(line+9, "%d %d %d", &numVertices[0], &numVertices[1], &numVertices[2]) !=3)
							Misc::throwStdErr("StructuredGocadFile::load: invalid grid size in line %u in grid file %s", lineIndex, argIt->c_str());
						}
					else
						Misc::throwStdErr("StructuredGocadFile::load: missing grid size in line %u in grid file %s", lineIndex, argIt->c_str());
					break;
				...
				case 1:
					if(strncasecmp(line,"coordinate", 10) == 0 && (line[10] == ' ' || line[10] == '\t'))
						...
						if(modeEndPtr-modeStartPtr == 9 && strncasecmp(modeStartPtr, "Cartesian" ,9) == 0)
							...
							Misc::throwStdErr("StructuredGocadFile::load: invalid coordinate mode in line %u in grid file %s", lineIndex, argIt->c_str());
						}
					else
						Misc::throwStdErr("StructuredGocadFile::load: missing coordinate mode in line %u in grid file %s", lineIndex, argIt->c_str());
					break;
				}
		...
		if(line[0] != '#')
			...
			if(sphericalCoordinates)
				...
				if(sscanf(line,"%lf %lf %lf", &longitude, &latitude, &height)!=3)
					Misc::throwStdErr("StructuredGocadFile::load: Invalid spherical vertex coordinate in line %u in grid file %s",lineIndex,argIt->c_str());
            ...
            std::cout<<"Converted Longitude: "<<vertex[0]<< " Latitude: "<<vertex[1]<<" Height: "<<vertex[2]<<"\n"<<std::flush;
				...
				if(sscanf(line, "%f %f %f", &vertex[0], &vertex[1], &vertex[2]) != 3)
					Misc::throwStdErr("StructuredGocadFile::load: Invalid Cartesian vertex coordinate in line %u in grid file %s", lineIndex, argIt->c_str());
				}
	...
	for(++argIt; argIt != args.end(); ++argIt)
		...
		if(strcasecmp(argIt->c_str(), "-log") == 0)
			...
			while(parsedHeaderLines < 2)
				...
				/* Check if the file is already over: */
				if(sliceFile.eof())
					Misc::throwStdErr("StructuredGocadFile::load: early end-of-file in slice file %s", argIt->c_str());
						...
						case 0:
							if(strncasecmp(line, "gridsize", 8) == 0 && (line[8] == ' ' || line[8] == '\t'))
								...
								if(sscanf(line+9, "%d %d %d", &sliceNumVertices[0], &sliceNumVertices[1], &sliceNumVertices[2]) != 3)
									Misc::throwStdErr("StructuredGocadFile::load: invalid grid size in line %u in slice file %s", lineIndex,argIt->c_str());
								
								/* Check if the slice grid size matches the grid size: */
								if(sliceNumVertices != numVertices)
									Misc::throwStdErr("StructuredGocadFile::load: mismatching grid size in slice file %s", argIt->c_str());
								}
							else
								Misc::throwStdErr("StructuredGocadFile::load: missing grid size in line %u in slice file %s", lineIndex,argIt->c_str());
							break;
							...
							if(strncasecmp(line, "scalar", 6) == 0 && (line[6] == ' ' || line[6] == '\t'))
								...
								if(nameStartPtr != nameEndPtr)
									...
									*nameEndPtr = '\0';
									...
									Misc::throwStdErr("StructuredGocadFile::load: empty vector variable name in line %u in slice file %s", lineIndex, argIt->c_str());
								}
								...
								Misc::throwStdErr("StructuredGocadFile::load: missing value type in line %u in slice file %s", lineIndex, argIt->c_str());
							break;
						}
				...
				if(line[0] != '#')
					...
					if(vectorValue)
						...
						if(sphericalCoordinates)
							...
							double longitude, latitude, height;
							if(sscanf(line, "%lf %lf %lf", &longitude, &latitude, &height) != 3)
								Misc::throwStdErr("StructuredGocadFile::load: Invalid spherical vector attribute in line %u in slice file %s", lineIndex, argIt->c_str());
							...
							if(sscanf(line, "%f %f %f", &vector[0], &vector[1], &vector[2]) != 3)
								Misc::throwStdErr("StructuredGocadFile::load: Invalid Cartesian vector attribute in line %u in slice file %s", lineIndex, argIt->c_str());
							}
						...
						if(logNextScalar)
							...
							if(sscanf(line, "%lf", &value) != 1)
								Misc::throwStdErr("StructuredGocadFile::load: Invalid logarithmic scalar vertex attribute in line %u in slice file %s",lineIndex,argIt->c_str());
							...
							if(sscanf(line, "%f", &dataSet.getVertexValue(sliceIndex, index)) != 1)
								Misc::throwStdErr("StructuredGocadFile::load: Invalid scalar vertex attribute in line %u in slice file %s", lineIndex, argIt->c_str());
							}
						}
...
extern "C" Visualization::Abstract::Module* createFactory(Plugins::FactoryManager<Visualization::Abstract::Module>& manager)
	...
	Visualization::Concrete::StructuredGocadFile* module = new Visualization::Concrete::StructuredGocadFile();
